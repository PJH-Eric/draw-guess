...
(function (w) {
  ...
  var lastError = '';
  /* 伺服器時間與本機時間的差。倒數是用伺服器給的 deadline 算的，
     本機時鐘不準時（常見）也不會讓倒數整個跑掉。 */
  var timeOffset = 0;
  ...
  function connect(identity) {
    ...
    return loadScript().then(function () {
      ...
      SERVER_EVENTS.forEach(function (evt) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      var finish = function () {
        ...
        socket.emit('hello', { clientId: identity.clientId, name: identity.name }, function (res) {
          ...
          clearTimeout(timer);
          if (res && res.ok) {
            if (res.serverTime) timeOffset = res.serverTime - Date.now();
            setStatus('connected');
            resolve(socket);
          }
  ...
  w.Online = {
    ...
    /** 換算成伺服器時間，用來算倒數 */
    now: function () { return Date.now() + timeOffset; },
