 ...
 * 真正的遊戲規則一律轉交 Rules，電腦對手的決策一律轉交 AI，
 * 兩邊跟單機模式用的是同一份程式。
 *
 ...
 *
 * 房間生命週期以「實體玩家」為準：
 *   實體玩家 = 坐在玩家席的真人。AI 席位與觀戰者都不算。
 *   實體玩家歸零（最後一個真人離開、退出、或斷線超過保留時間）→ 立刻關房，
 *   邀請失效、計時器與 AI 工作停掉、觀戰者收到「房間已結束」。
 *   關掉之後不會因為重新連線或舊邀請復活。
 *
...
class Room {
  constructor(store, opts) {
    ...
    this.phase = 'waiting';
    this.state = null;
    this.director = AI.createDirector();
  ...
  seatsTaken() { return this.humanPlayers().length + this.aiSeats.size; }
  openSeats() { return Math.max(0, this.settings.maxPlayers - this.seatsTaken()); }

  /** 送進 Rules 的名單：真人玩家 + AI 席位 */
  roster() {
    const list = this.humanPlayers()
      .sort((a, b) => a.joinedAt - b.joinedAt)
      .map((m) => ({ id: m.id, name: m.name, ai: null }));
    for (const a of this.aiSeats.values()) list.push({ id: a.id, name: a.name, ai: a.level });
    return list;
  }
  ...
  join(id, opts) {
    ...
    /* 想當玩家但沒位子（或已經開打）→ 明白地轉為觀戰，不默默佔位 */
    let downgraded = false;
    if (wantRole === 'player' && (this.openSeats() <= 0 || this.phase === 'playing')) {
      wantRole = 'spectator';
      downgraded = true;
    }
  ...
  becomePlayer(id) {
    ...
    if (this.phase === 'playing') return err('對局進行中不能下場，等這一局結束吧。', 'playing');
  ...
  addAi(hostId, level) {
    if (!this.isHost(hostId)) return err('只有房主可以安排電腦對手。', 'forbidden');
    if (this.phase === 'playing') return err('對局進行中不能更動席位。');
    if (!AI.LEVELS[level]) return err('沒有這個難度。');
    if (this.openSeats() <= 0) return err('席位已經滿了。', 'full');
    this.aiSerial += 1;
    const id = 'ai' + this.aiSerial;
    const seat = { id, name: '電腦' + this.aiSerial + '號（' + AI.levelOf(level).label + '）', level };
    this.aiSeats.set(id, seat);
    this.touch();
    return ok({ seat });
  }

  removeAi(hostId, aiId) {
    if (!this.isHost(hostId)) return err('只有房主可以移除電腦對手。', 'forbidden');
    if (this.phase === 'playing') return err('對局進行中不能更動席位。');
    const seat = this.aiSeats.get(String(aiId || ''));
    if (!seat) return err('找不到這個電腦席位。', 'gone');
    this.aiSeats.delete(seat.id);
    this.touch();
    return ok({ seat });
  }

  setAiLevel(hostId, aiId, level) {
    if (!this.isHost(hostId)) return err('只有房主可以調整電腦難度。', 'forbidden');
    if (this.phase === 'playing') return err('對局進行中不能更動席位。');
    const seat = this.aiSeats.get(String(aiId || ''));
    if (!seat) return err('找不到這個電腦席位。', 'gone');
    if (!AI.LEVELS[level]) return err('沒有這個難度。');
    seat.level = level;
    seat.name = seat.name.replace(/（.*）$/, '（' + AI.levelOf(level).label + '）');
    this.touch();
    return ok({ seat });
  }
  ...
  /** 電腦對手的猜題：跳過冷卻，但完全走同一套 Rules.guess */
  aiGuess(aiId, text, now) {
    if (this.phase !== 'playing' || !this.state) return err('現在沒有進行中的對局。', 'phase');
    const seat = this.aiSeats.get(aiId);
    if (!seat) return err('找不到這個電腦席位。', 'gone');
    const clean = sanitizeText(text, this.store.opts.guessMax);
    if (!clean) return err('猜測是空的。', 'empty');

    const g = Rules.guess(this.state, aiId, clean, now);
    if (!g.ok) return g;
    if (g.verdict === 'hit') {
      this.touch();
      this.noteCorrect(seat, g, now);
      return ok({ kind: 'hit', points: g.points, order: g.order, allDone: g.allDone, name: seat.name });
    }
    return ok({ kind: 'miss', entry: this.pushFeed(this.guessEntry(seat.name, seat.id, 'ai', clean, now)) });
  }
  ...
  /**
   * 每秒（或每次有人行動後）呼叫一次：推進規則時間，再讓 AI 出手。
  ...
  tick(now) {
    ...
    /* AI 席位：選題、作畫、猜題都走跟人類一樣的入口 */
    AI.drive(this.director, this.state, now, this.seed, {
      pick: (aiId, wordId) => {
        const r = Rules.pickWord(this.state, aiId, wordId, now);
        if (r.ok) { this.noteWordPicked(now, false); out.events.push({ type: 'aipick', aiId }); this.touch(); }
      },
      stroke: (aiId, stroke) => {
        const r = Rules.addStroke(this.state, aiId, stroke);
        if (r.ok) { out.strokes.push(r.stroke); this.touch(); }
      },
      hint: (aiId) => {
        const r = Rules.giveHint(this.state, aiId, now);
        if (r.ok) { this.noteHint(r, now); out.events.push({ type: 'hint', step: r.step, mask: r.mask }); }
      },
      guess: (aiId, text) => {
        const r = this.aiGuess(aiId, text, now);
        if (r.ok && r.kind === 'miss') out.feed.push(r.entry);
        if (r.ok && r.kind === 'hit') out.events.push({ type: 'aihit', aiId });
      }
    });

    /* AI 出手後可能剛好湊齊「全部猜中」，再推一次時間 */
    const t2 = Rules.tick(this.state, now);
    for (const ev of t2.events) this.applyEvent(ev, now);
    out.events.push(...t2.events);
    if (this.state.over) this.finishGame(now);
  ...
  brief() {
    return {
      ...
      ai: [...this.aiSeats.values()].map((a) => ({ id: a.id, level: a.level })),
  ...
  viewFor(id, now) {
    ...
    return {
      room: {
        ...
        aiSeats: [...this.aiSeats.values()].map((a) => ({ id: a.id, name: a.name, level: a.level })),
      ...
      you: m ? {
        ...
        can: {
          ...
          becomePlayer: m.role === 'spectator' && this.phase !== 'playing' && this.openSeats() > 0,
...
class RoomStore {
  ...
  create(id, { name, roomName, private: isPrivate, settings, aiLevels, now }) {
    ...
    const levels = Array.isArray(aiLevels) ? aiLevels.slice(0, room.settings.maxPlayers - 1) : [];
    for (const level of levels) {
      const added = room.addAi(id, String(level || 'normal'));
